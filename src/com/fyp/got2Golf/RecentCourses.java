...
//Store the previous round in the phone and allow user to continue
//From MainMenu
//To ScorecardIndividual
public class RecentCourses extends ListActivity{
	String [] recentArray;

	ListView listViewCourses;
	private DBAdapter mDbHelper;
	private Cursor mDbCourseCursor;
	...
	public void onCreate(Bundle savedInstanceState) {
		...
		setContentView(R.layout.recentcourses);
		setRequestedOrientation(ActivityInfo.SCREEN_ORIENTATION_PORTRAIT);
		//TODO
		//Set shared pref
		SharedPreferences recentPlayed = getSharedPreferences("myPrefs", MODE_PRIVATE);
		//recover all recents from shared pref
		recentlyPlayed = recentPlayed.getString("recentlyPlayed", "");
		//split all recent String into array
		recentArray = recentlyPlayed.split(",");

		mDbHelper = new DBAdapter(this);
		mDbHelper.open();
		//loop through the entire array of recently played courses, each time passing the course ID as a paramter to the fill data method
		for (int i =0; i<recentArray.length; i++)
		{
			try{
				int recentParameter = Integer.parseInt(recentArray[i]);
				fillData(recentParameter);
			}
			catch (Exception ex)
			{
	...
	private void fillData(int recentParameter) {
		// Get all of the notes from the database and create the item list
		//TODO create query to only find courses that match the correct id.
		int paramRecent = recentParameter;
		//Cursor tempCursor = mDbCourseCursor;
		Cursor mDbCourseCursor = mDbHelper.getCourse(paramRecent);
